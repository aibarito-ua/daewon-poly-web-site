...
import { goOutlineDraft } from "../controller/Navigate";
import useControlAlertStore from "../../../../store/useControlAlertStore";
import { showConfirmPopup } from "../controller/AlertModalController";

const OutlineDraftFormatBtn = (): JSX.Element => {
    ...
    <div className="flex items-center h-[90px]">
      <div className='ocr-draft-outline-btn div-to-button-hover-effect' onClick={handleClick}>
        <div className='w-[36px] h-[36px]'>
          <div className="ocr-draft-outline-btn-icon bg-no-repeat bg-contain w-[36px] h-[36px]" />
        </div>
        <span className='ocr-draft-outline-btn-text w-[60px]'>Outline</span>
      </div>
    </div>
  );
};

export default OutlineDraftFormatBtn;
...
async function fetchDataAndGoPage(unit: string, draft: string, navigate: NavigateFunction) {
  ...
  // 저장 페이지 타입이 without outline 경우 input_content 리셋 후 이동
  const writingData  = sparkWritingData[getUnitIndex(unit)];
  if (writingData.draft_1_page_outline_type === 'WO') {
    ...
    return goOutlineDraft(unit, draft, navigate);
  }

  // 저장 페이지 타입이 outline인 경우 데이터 fetch 후 이동
  ...
  goOutlineDraft(unit, draft, navigate);
}